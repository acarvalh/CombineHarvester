...
if not options.original == "none" :
    ...
    ###
    print ("original readFrom ", readFrom, fileOrig)
    ...
    print("len(list_cats)", len(list_cats), binToRead)
    if not binToRead == "none" :
        if len(list_cats) <= 1 :
...
if HH :
    colorsH = [1, 2, 8, 5, 6, 7]
    histogramsHH = [ROOT.TH1F(), ROOT.TH1F(), ROOT.TH1F(), ROOT.TH1F(), ROOT.TH1F(), ROOT.TH1F()]
...
if do_bottom :
    ...
    dumb = hist_total_err.Draw("e2,same")
    del dumb
    if options.unblind :
        if not options.fromHavester :
